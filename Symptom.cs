...
namespace LethalDiseases
{
    public class Symptom(string name, string description, SymptomType symptomType, float rarity, Func<Disease, StatusEffect> effect)
    {
        ...
        {
            ConfigEntry<bool> cfgEnabled = Plugin.Instance.Config.Bind<bool>($"{symptom.symptomType.ToString()} Symptoms", symptom.name, true, $"Whether the symptom '{symptom.name}' can appear in diseases\n\n{symptom.description}");
